...
from dlas.models.image_generation.stylegan.stylegan2_lucidrains import \
    gradient_penalty
from dlas.trainer.inject import Injector
from dlas.trainer.losses import (ConfigurableLoss, GANLoss,
                                 extract_params_from_state,
                                 get_basic_criterion_for_name)
...
class RecurrentImageGeneratorSequenceInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.input_lq_index = opt['input_lq_index'] if 'input_lq_index' in opt.keys(
        ) else 0
        self.recurrent_index = opt['recurrent_index']
        self.output_hq_index = opt['output_hq_index'] if 'output_hq_index' in opt.keys(
        ) else 0
        self.output_recurrent_index = opt['output_recurrent_index'] if 'output_recurrent_index' in opt.keys(
        ) else self.output_hq_index
        self.scale = opt['scale']
        self.resample = Resample2d()
        self.flow_key = opt['flow_input_key'] if 'flow_input_key' in opt.keys(
        ) else None
        # Use this to specify inputs that will be used in the first teco iteration, the rest will use 'in'.
        self.first_inputs = opt['first_inputs'] if 'first_inputs' in opt.keys(
        ) else opt['in']
        self.do_backwards = opt['do_backwards'] if 'do_backwards' in opt.keys(
        ) else True
        # When True, recurrent_index is not touched for the first iteration, allowing you to specify what is fed in. When False, zeros are fed into the recurrent index.
        self.hq_recurrent = opt['hq_recurrent'] if 'hq_recurrent' in opt.keys(
        ) else False
        self.hq_batched_output_key = opt['hq_batched_key'] if 'hq_batched_key' in opt.keys(
        ) else None
    ...
    def forward(self, state):
        ...
        for i in range(f):
            if first_step:
                ...
                if self.hq_recurrent:
                    ...
                    recurrent_input = torch.zeros_like(
                ...
                with torch.no_grad() and autocast(enabled=False):
                    ...
                    reduced_recurrent = F.interpolate(
                        hq_recurrent, scale_factor=1/self.scale, mode='bicubic')
                    flow_input = torch.stack(
                        [flow_input, reduced_recurrent], dim=2).float()
                    flowfield = flow(flow_input)
                    if recurrent_input.shape[-1] != flow_input.shape[-1]:
                        flowfield = F.interpolate(
                            flowfield, scale_factor=self.scale, mode='bicubic')
                    recurrent_input = self.resample(
                        recurrent_input.float(), flowfield)
            input[self.recurrent_index] = recurrent_input
            if self.env['step'] % 50 == 0:
                # Only debug this if we're dealing with images.
                if input[self.input_lq_index].shape[1] == 3:
                    self.produce_teco_visual_debugs(
                        input[self.input_lq_index], input[self.hq_recurrent], debug_index)
                    debug_index += 1
            ...
            for i in it:
                ...
                with torch.no_grad():
                    with autocast(enabled=False):
                        ...
                        if recurrent_input.shape[-1] != flow_input.shape[-1]:
                            flowfield = F.interpolate(
                                flow(flow_input), scale_factor=self.scale, mode='bicubic')
                        recurrent_input = self.resample(
                            recurrent_input.float(), flowfield)
                input[self.recurrent_index] = recurrent_input
                if self.env['step'] % 50 == 0:
                    ...
                    if input[self.input_lq_index].shape[1] == 3:
                        self.produce_teco_visual_debugs(
                            input[self.input_lq_index], input[self.recurrent_index], debug_index)
                        debug_index += 1
        ...
        if self.hq_batched_output_key is not None:
            final_results[self.hq_batched_output_key] = state['hq'].clone().permute(
                1, 0, 2, 3, 4).reshape(b*s, c, h, w)
        for k, v in results.items():
            final_results[k] = torch.stack(v, dim=1)
            # Only include the original sequence - this output is generally used to compare against HQ.
            final_results[k + "_batched"] = torch.cat(v[:s], dim=0)
        return final_results

    def produce_teco_visual_debugs(self, gen_input, gen_recurrent, it):
        if self.env['rank'] > 0:
            return
        base_path = osp.join(self.env['base_path'], "../../models",
                             "visual_dbg", "teco_geninput", str(self.env['step']))
        os.makedirs(base_path, exist_ok=True)
        torchvision.utils.save_image(
            gen_input.float(), osp.join(base_path, "%s_img.png" % (it,)))
        torchvision.utils.save_image(gen_recurrent.float(), osp.join(
            base_path, "%s_recurrent.png" % (it,)))
...
class FlowAdjustment(Injector):
    ...
    def forward(self, state):
        with autocast(enabled=False):
            ...
            flowed = F.interpolate(
                state[self.flowed], size=flow_target.shape[2:], mode='bicubic')
            flow_input = torch.stack([flow_target, flowed], dim=2).float()
            flowfield = F.interpolate(
                flow(flow_input), size=state[self.flowed].shape[2:], mode='bicubic')
...
def create_teco_discriminator_sextuplet(input_list, lr_imgs, scale, index, flow_gen, resampler, margin):
    ...
    with autocast(enabled=False):
        ...
        first_flow = flow_gen(torch.stack(
            [triplet[:, 1], triplet[:, 0]], dim=2))
        last_flow = flow_gen(torch.stack(
            [triplet[:, 1], triplet[:, 2]], dim=2))
        ...
        # 3*6 is essentially an assertion here.
        combined = combined.view(b, 3*6, h, w)
    # Apply margin
...
def create_all_discriminator_sextuplets(input_list, lr_imgs, scale, total, flow_gen, resampler, margin):
    with autocast(enabled=False):
        ...
        flux_doubles_forward = [torch.stack(
            [input_list[:, i], input_list[:, i+1]], dim=2) for i in range(1, total+1)]
        flux_doubles_backward = [torch.stack(
            [input_list[:, i], input_list[:, i-1]], dim=2) for i in range(1, total+1)]
...
class TecoGanLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        # Per the tecogan paper, the GAN loss only pays attention to an inner part of the image with the margin removed, to get rid of artifacts resulting from flow errors.
        self.margin = opt['margin']
        self.ff = opt['fast_forward'] if 'fast_forward' in opt.keys() else False
        self.noise = opt['noise'] if 'noise' in opt.keys() else 0
        self.gradient_penalty = opt['gradient_penalty'] if 'gradient_penalty' in opt.keys(
        ) else False
    ...
    def lowmem_forward(self, state):
        ...
        for i in range(sequence_len - 2):
            real_sext = create_teco_discriminator_sextuplet(
                real, lr, self.scale, i, flow_gen, self.resampler, self.margin)
            if self.gradient_penalty:
                real_sext.requires_grad_()
            fake_sext = create_teco_discriminator_sextuplet(
                fake, lr, self.scale, i, flow_gen, self.resampler, self.margin)
    ...
    def fast_forward(self, state):
        ...
        combined_fake_sext = create_all_discriminator_sextuplets(fake, lr, self.scale, sequence_len - 2, flow_gen,
                                                                 self.resampler, self.margin)
        l_total, d_real = self.compute_loss(
            combined_real_sext, combined_fake_sext)
    ...
    def produce_teco_visual_debugs(self, sext, lbl, it):
        if self.env['rank'] > 0:
            return
        base_path = osp.join(self.env['base_path'], "../../models",
                             "visual_dbg", "teco_sext", str(self.env['step']), lbl)
        os.makedirs(base_path, exist_ok=True)
        lbls = ['img_a', 'img_b', 'img_c', 'flow_a', 'flow_b', 'flow_c']
        for i in range(6):
            torchvision.utils.save_image(
                sext[:, i*3:(i+1)*3, :, :].float(), osp.join(base_path, "%s_%s.png" % (it, lbls[i])))
...
class PingPongLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.opt = opt
        self.criterion = get_basic_criterion_for_name(
            opt['criterion'], env['device'])
    ...
    def produce_teco_visual_debugs(self, imglist):
        if self.env['rank'] > 0:
            return
        base_path = osp.join(self.env['base_path'], "../../models",
                             "visual_dbg", "teco_pingpong", str(self.env['step']))
        os.makedirs(base_path, exist_ok=True)
        cnt = imglist.shape[1]
        for i in range(cnt):
            img = imglist[:, i]
            torchvision.utils.save_image(
                img.float(), osp.join(base_path, "%s.png" % (i, )))
    ...
    def produce_teco_visual_debugs2(self, imga, imgb, i):
        ...
        os.makedirs(base_path, exist_ok=True)
        torchvision.utils.save_image(
            imga.float(), osp.join(base_path, "%s_a.png" % (i, )))
        torchvision.utils.save_image(
            imgb.float(), osp.join(base_path, "%s_b.png" % (i, )))
