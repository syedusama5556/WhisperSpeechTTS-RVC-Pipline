...
from dlas.data.images.multiscale_dataset import \
    build_multiscale_patch_index_map
...
class ProgressiveGeneratorInjector(Injector):
    def __init__(self, opt, env):
        ...
        # The key where HQ images corresponding with generated images are stored.
        self.hq_output_key = opt['hq_output']
        ...
        # Number of input branches to randomly choose for generation. This defines the output shape.
        self.number_branches = opt['num_branches']
    ...
    def forward(self, state):
        ...
        if not isinstance(self.output, list):
            output = [self.output]
            self.output = output
        # A list of outputs produced by feeding each progressive lq input into the generator.
        results = {}
        results_hq = []
        ...
        base_hq_out, base_recurrent = self.feed_forward(
            gen, inputs, results, lq_inputs[:, 0], None)
        ...
        for chain in input_chains:
            ...
            for link in chain:  # Remember, `link` is a MultiscaleTreeNode.
                ...
                if recurrent is not None:
                    recurrent = torch.nn.functional.interpolate(
                        recurrent[:, :, top:top+h//2, left:left+w//2], scale_factor=2, mode="nearest")
                ...
                recurrent_hq, recurrent = self.feed_forward(
                    gen, inputs, results, lq_input, recurrent)
                chain_output.append(recurrent_hq)
                results_hq.append(hq_inputs[:, link.index])

            if self.env['step'] % 50 == 0:
                self.produce_progressive_visual_debugs(
                    chain_input, chain_output, debug_index)
                debug_index += 1
    ...
    def produce_progressive_visual_debugs(self, chain_inputs, chain_outputs, it):
        ...
        base_path = osp.join(
            self.env['base_path'], "../../models", "visual_dbg", lbl, str(self.env['step']))
        os.makedirs(base_path, exist_ok=True)
        ind = 1
        for i, o in zip(chain_inputs, chain_outputs):
            torchvision.utils.save_image(i.float(), osp.join(
                base_path, "%s_%i_input.png" % (it, ind)))
            torchvision.utils.save_image(o.float(), osp.join(
                base_path, "%s_%i_output.png" % (it, ind)))
            ind += 1
