...
from dlas.trainer.injectors.gaussian_diffusion_injector import \
    GaussianDiffusionInferenceInjector
...
class SrDiffusionFidEvaluator(evaluator.Evaluator):
    def __init__(self, model, opt_eval, env):
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            self.sampler = DistributedSampler(
                self.dataset, shuffle=False, drop_last=True)
        else:
    ...
    def perform_eval(self):
        ...
        dataloader = DataLoader(
            self.dataset, self.batch_sz, sampler=self.sampler, num_workers=0)
