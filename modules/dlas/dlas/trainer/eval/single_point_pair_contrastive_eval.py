...
from dlas.data.images.image_pair_with_corresponding_points_dataset import \
    ImagePairWithCorrespondingPointsDataset
...
class SinglePointPairContrastiveEval(evaluator.Evaluator):
    def __init__(self, model, opt_eval, env):
        ...
        self.similar_set = DataLoader(ImagePairWithCorrespondingPointsDataset(
            opt_eval['similar_set_args']), shuffle=False, batch_size=self.batch_sz)
        self.dissimilar_set = DataLoader(ImagePairWithCorrespondingPointsDataset(
            opt_eval['dissimilar_set_args']), shuffle=False, batch_size=self.batch_sz)
    ...
    def get_l2_score(self, dl, dev):
        ...
        for i, data in tqdm(enumerate(dl)):
            latent1 = self.model(img=data['img1'].to(
                dev), pos=torch.stack(data['coords1'], dim=1).to(dev))
            latent2 = self.model(img=data['img2'].to(
                dev), pos=torch.stack(data['coords2'], dim=1).to(dev))
    ...
    def perform_eval(self):
        ...
        with torch.no_grad():
            ...
            print(
                f"Eval done. val_similar_lq: {similars.item()}; val_dissimilar_l2: {dissimilars.item()}; val_diff: {diff}")
        self.model.train()
