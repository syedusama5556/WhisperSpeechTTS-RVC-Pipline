...
from dlas.data.audio.paired_voice_audio_dataset import load_tsv_aligned_codes
...
from dlas.models.audio.tts.tacotron2 import text_to_sequence
from dlas.models.clip.mel_text_clip import MelTextCLIP
from dlas.scripts.audio.gen.speech_synthesis_utils import (
    convert_mel_to_codes, load_clvp, load_discrete_vocoder_diffuser,
    load_speech_dvae, load_univnet_vocoder, wav_to_mel, wav_to_univnet_mel)
from dlas.trainer.injectors.audio_injectors import (
    TorchMelSpectrogramInjector, denormalize_mel, normalize_mel)
from dlas.utils.util import (ceil_multiple, load_model_from_config, opt_get,
                             pad_or_truncate)
...
class AudioDiffusionFid(evaluator.Evaluator):
    ...
    def __init__(self, model, opt_eval, env):
        ...
        if distributed.is_initialized() and distributed.get_world_size() > 1:
            # One batch element per GPU.
            self.skip = distributed.get_world_size()
        else:
            self.skip = 1
        diffusion_steps = opt_get(opt_eval, ['diffusion_steps'], 50)
        diffusion_schedule = opt_get(env['opt'], [
                                     'steps', 'generator', 'injectors', 'diffusion', 'beta_schedule', 'schedule_name'], None)
        if diffusion_schedule is None:
            print(
                "Unable to infer diffusion schedule from master options. Getting it from eval (or guessing).")
            diffusion_schedule = opt_get(
                opt_eval, ['diffusion_schedule'], 'cosine')
        conditioning_free_diffusion_enabled = opt_get(
            opt_eval, ['conditioning_free'], False)
        ...
        if mode == 'tts':
            ...
            mel_means, self.mel_max, self.mel_min, mel_stds, mel_vars = torch.load(
                '../experiments/univnet_mel_norms.pth')
            self.local_modules['dvae'] = load_speech_dvae().cpu()
            self.local_modules['vocoder'] = load_univnet_vocoder().cpu()
    ...
    def perform_diffusion_tts(self, audio, codes, text, sample_rate=5500):
        real_resampled = torchaudio.functional.resample(
            audio, 22050, sample_rate).unsqueeze(0)
    ...
    def perform_original_diffusion_vocoder(self, audio, codes, text, sample_rate=11025):
        ...
        real_resampled = torchaudio.functional.resample(
            real_resampled.squeeze(0), sample_rate, 5500).unsqueeze(0)
        gen = torchaudio.functional.resample(
            gen.squeeze(0), sample_rate, 5500).unsqueeze(0)
    ...
    def tts9_get_autoregressive_codes(self, mel, text):
        mel_codes = convert_mel_to_codes(self.local_modules['dvae'], mel)
        text_codes = torch.LongTensor(
            self.bpe_tokenizer.encode(text)).unsqueeze(0).to(mel.device)
        ...
        auto_latents = self.local_modules['autoregressive'](cond_inputs, text_codes,
                                                            torch.tensor(
                                                                [text_codes.shape[-1]], device=mel.device),
                                                            mel_codes,
                                                            torch.tensor(
                                                                [mel_codes.shape[-1]*mlc], device=mel.device),
    ...
    def perform_diffusion_tts9_mel_from_codes(self, audio, codes, text):
        ...
        real_resampled = torchaudio.functional.resample(
            audio, 22050, SAMPLE_RATE).unsqueeze(0)
        # to be used for a conditioning input, but also guides output shape.
        univnet_mel = wav_to_univnet_mel(
            real_resampled, do_normalization=False)
    ...
    def perform_diffusion_ctc(self, audio, codes, text):
        SAMPLE_RATE = 24000
        text_codes = torch.LongTensor(
            self.bpe_tokenizer.encode(text)).unsqueeze(0).to(audio.device)
        ...
        real_dec = self.local_modules['vocoder'].inference(
            denormalize_mel(univnet_mel))
        return gen_wav.float(), real_dec, gen_mel, univnet_mel, SAMPLE_RATE
    ...
    def perform_diffusion_tfd(self, audio, codes, text):
        ...
        gen_wav = self.local_modules['vocoder'].inference(
            denormalize_mel(gen_mel))
        real_dec = self.local_modules['vocoder'].inference(
            denormalize_mel(umel))
    ...
    def perform_diffusion_tfd_ar_prior(self, audio, codes, text):
        ...
        text_codes = torch.LongTensor(
            self.bpe_tokenizer.encode(text)).unsqueeze(0).to(vmel.device)
        cond_inputs = pad_or_truncate(vmel, 132300//256).unsqueeze(1)
        mlc = self.local_modules['autoregressive'].mel_length_compression
        auto_latents = self.local_modules['autoregressive'](cond_inputs, text_codes,
                                                            torch.tensor(
                                                                [text_codes.shape[-1]], device=vmel.device),
                                                            mel_codes,
                                                            torch.tensor(
                                                                [mel_codes.shape[-1]*mlc], device=vmel.device),
                                                            text_first=True, raw_mels=None, return_latent=True)
    ...
    def load_projector(self):
        ...
        weights = torch.load(
            '../experiments/clip_text_to_voice_for_speech_fid.pth')
    ...
    def project(self, projector, sample, sample_rate):
        ...
        # Getting rid of the batch dimension means it's just [hidden_dim]
        return projector.get_speech_projection(mel).squeeze(0)
    ...
    def intelligibility_loss(self, w2v, sample, real_sample, sample_rate, real_text):
        ...
        """
        text_codes = torch.tensor(text_to_sequence(
            real_text), device=sample.device)
        results = []
    ...
    def perform_eval(self):
        save_path = osp.join(
            self.env['base_path'], "../", "audio_eval", str(self.env["step"]))
        ...
        with torch.no_grad():
            ...
            for i in tqdm(list(range(0, len(self.data), self.skip))):
                path, text, codes = self.data[(
                    i + self.env['rank']) % len(self.data)]
                audio = load_audio(path, 22050).to(self.dev)
                codes = codes.to(self.dev)
                sample, ref, gen_mel, ref_mel, sample_rate = self.diffusion_fn(
                    audio, codes, text)

                # Store on CPU to avoid wasting GPU memory.
                gen_projections.append(self.project(
                    projector, sample, sample_rate).cpu())
                real_projections.append(self.project(
                    projector, ref, sample_rate).cpu())
                intelligibility_losses.append(self.intelligibility_loss(
                    w2v, sample, ref, sample_rate, text))

                torchvision.utils.save_image((gen_mel.unsqueeze(
                    1) + 1) / 2, os.path.join(save_path, f'{self.env["rank"]}_{i}_mel.png'))
                torchvision.utils.save_image((ref_mel.unsqueeze(
                    1) + 1) / 2, os.path.join(save_path, f'{self.env["rank"]}_{i}_mel_target.png'))
                torchaudio.save(os.path.join(
                    save_path, f"{self.env['rank']}_{i}_gen.wav"), sample.squeeze(0).cpu(), sample_rate)
                torchaudio.save(os.path.join(
                    save_path, f"{self.env['rank']}_{i}_real.wav"), ref.squeeze(0).cpu(), sample_rate)
            gen_projections = torch.stack(gen_projections, dim=0)
            real_projections = torch.stack(real_projections, dim=0)
            intelligibility_loss = torch.stack(
                intelligibility_losses, dim=0).mean()
            frechet_distance = torch.tensor(self.compute_frechet_distance(
                gen_projections, real_projections), device=self.env['device'])
...
if __name__ == '__main__':
    ...
    env = {'rank': 0, 'base_path': 'D:\\tmp\\test_eval',
           'step': 101, 'device': 'cuda', 'opt': {}}
    eval = AudioDiffusionFid(diffusion, opt_eval, env)
    print(eval.perform_eval())
