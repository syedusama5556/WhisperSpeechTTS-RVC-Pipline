...
import torch
import torchvision
import trdlas.ainer.eval.evaluator as evaluator
...
class StyleTransferEvaluator(evaluator.Evaluator):
    def __init__(self, model, opt_eval, env):
        ...
        # Not needed if using 'imgnoise' input.
        self.latent_dim = opt_get(opt_eval, ['latent_dim'], 512)
    ...
    def perform_eval(self):
        ...
        for i in range(self.batches_per_eval):
            if self.noise_type == 'imgnoise':
                batch = torch.FloatTensor(self.batch_sz, 3, self.im_sz, self.im_sz).uniform_(
                    0., 1.).to(self.env['device'])
            elif self.noise_type == 'stylenoise':
                batch = torch.randn(self.batch_sz, self.latent_dim).to(
                    self.env['device'])
