...
class VitLatent(nn.Module):
    def __init__(self, top_dim, hidden_dim, depth, dropout=.1):
        ...
        self.upper = nn.Sequential(nn.Conv2d(3, top_dim, kernel_size=7, padding=3, stride=2),
                                   ResBlock(top_dim, use_conv=True,
                                            dropout=dropout),
                                   ResBlock(top_dim, out_channels=top_dim*2,
                                            down=True, use_conv=True, dropout=dropout),
                                   ResBlock(top_dim*2, use_conv=True,
                                            dropout=dropout),
                                   ResBlock(top_dim*2, out_channels=top_dim*4,
                                            down=True, use_conv=True, dropout=dropout),
                                   ResBlock(top_dim*4, use_conv=True,
                                            dropout=dropout),
                                   ResBlock(top_dim*4, out_channels=hidden_dim,
                                            down=True, use_conv=True, dropout=dropout),
    ...
    def forward(self, x1, x2):
        ...
        def csim(h1, h2):
            ...
            sim = F.cosine_similarity(
                h1.unsqueeze(0), h2.unsqueeze(1).detach(), 2)
