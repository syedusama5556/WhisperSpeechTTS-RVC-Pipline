...
class VoiceCLIP(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            use_xformers=False,
            clip_mels=False,
            # Default is approximately .5sec with default mel specs.
            min_mel_size=10,
    ...
    def forward(
        ...
        if self.training:
            if self.clip_mels:
                ...
                speech_tokens = speech_tokens[:,
                                              :self.min_mel_size+randint(0, margin)]
                # Disable voice masking in this case.
                voice_mask = torch.ones_like(speech_tokens.float()).bool()
            else:
                voice_mask = torch.rand_like(
                    speech_tokens.float()) > self.voice_mask_percentage
            text_mask = torch.rand_like(
                text.float()) > self.text_mask_percentage
        else:
            text_mask = torch.ones_like(text.float()).bool()
            voice_mask = torch.ones_like(speech_tokens.float()).bool()
        ...
        if self.distributed_collect:
            ...
            collective[torch.distributed.get_rank()] = text_latents
            text_latents = torch.cat(collective, dim=0)
            collective = [torch.zeros_like(speech_latents) for _ in range(
                torch.distributed.get_world_size())]
            # For gradient propagation.
            collective[torch.distributed.get_rank()] = speech_latents
...
if __name__ == '__main__':
    clip = VoiceCLIP(text_mask_percentage=.2,
                     voice_mask_percentage=.2, use_xformers=True)
