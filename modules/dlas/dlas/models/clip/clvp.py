...
from torch import distributed, einsum
...
class CLVP(nn.Module):
    ...
    def __init__(
        ...
        self.conditioning_transformer = CollapsingTransformer(
            model_dim, model_dim*2, transformer_heads, dropout, conditioning_enc_depth, 0)
        self.masked_conditioning_latent = nn.Parameter(
            torch.randn(1, model_dim*2), requires_grad=True)
        self.mask_conditioning_percentage = mask_conditioning_percentage

        # nn.Embedding
        self.text_emb = ml.Embedding(num_text_tokens, model_dim)
        self.text_transformer = CollapsingTransformer(
            model_dim, latent_dim, transformer_heads, dropout, text_enc_depth, text_mask_percentage, use_rms_scaleshift_norm=True)
    ...
    def forward(
        ...
        if random() < self.mask_conditioning_percentage:
            ...
            unused_params.extend(list(self.cond_emb.parameters(
            )) + list(self.conditioning_transformer.parameters()))
        else:
        ...
        enc_text = self.text_transformer(
            text_emb, norm_scale_shift_inp=enc_cond)
        ...
        text_latents, speech_latents = map(lambda t: F.normalize(
            t, p=2, dim=-1), (text_latents, speech_latents))
        temp = self.temperature.exp()

        if self.distributed_collect:
            collective = [torch.zeros_like(text_latents) for _ in range(
                torch.distributed.get_world_size())]
            torch.all_gather(collective, text_latents)
            text_latents = torch.cat(collective, dim=0)
            collective = [torch.zeros_like(speech_latents) for _ in range(
                torch.distributed.get_world_size())]
