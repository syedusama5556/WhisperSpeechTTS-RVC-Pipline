...
from torch import einsum, nn, broadcast_tensors

import dlas.torch_intermediary as ml
from dlas.models.lucidrains.dalle.attention import (Attention, SparseAttention,
                                                    SparseAxialCausalAttention,
                                                    SparseConvCausalAttention)
from dlas.models.lucidrains.dalle.reversible import (ReversibleSequence,
                                                     SequentialSequence)
...
class PreShiftToken(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        x_img = torch.cat(
            (x_img_shift_top, x_img_shift_left, *x_img_pass), dim=-1)
        ...
        *,
        ...
        for ind, sparse_attn, attn_type in zip(range(depth), sparse_layer, attn_type_layer):
            if attn_type == 'full':
                ...
                attn_class = partial(SparseAxialCausalAttention, seq_len=seq_len,
                                     axis=0, image_size=image_fmap_size, stable=stable)
            elif attn_type == 'axial_col':
                attn_class = partial(SparseAxialCausalAttention, seq_len=seq_len,
                                     axis=1, image_size=image_fmap_size, stable=stable)
            elif attn_type == 'conv_like':
                attn_class = partial(
                    SparseConvCausalAttention, seq_len=seq_len, image_size=image_fmap_size, stable=stable)
            elif attn_type == 'mlp':
            ...
            if attn_type != 'mlp':
                attn = attn_class(dim, causal=causal, seq_len=seq_len,
            ...
            if shift_tokens:
                attn, ff = map(lambda t: PreShiftToken(
                    t, image_size=image_fmap_size, seq_len=seq_len), (attn, ff))

            layers.append(nn.ModuleList([
                LayerScale(dim, ind + 1, PreNorm(dim,
                           attn, sandwich=sandwich_norm)),
                LayerScale(dim, ind + 1, PreNorm(dim,
                           ff, sandwich=sandwich_norm))
            ]))
        ...
        if rotary_emb:
            ...
            rot_dim = dim_head // 3
            img_seq_len = (image_fmap_size **
                           2) if image_fmap_size is not None else oned_fmap_size
            text_len = seq_len - img_seq_len + 1
            ...
            img_freqs_axial = img_axial_pos_emb(
                torch.linspace(-1, 1, steps=image_fmap_size if image_fmap_size is not None else oned_fmap_size))
            img_freqs = broadcast_tensors((rearrange(img_freqs_axial, 'i d -> i () d'),
            ...
            text_axial_freqs = img_axial_pos_emb(torch.full((text_len,), -10.))
