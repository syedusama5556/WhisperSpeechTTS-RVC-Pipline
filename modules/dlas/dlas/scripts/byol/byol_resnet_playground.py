...
from dlas.models.classifiers.resnet_with_checkpointing import resnet50
...
def produce_latent_dict(model):
    ...
    for batch in tqdm(dataloader):
        hq = batch['hq'].to('cuda')
        hq = F.interpolate(F.interpolate(hq, size=(16, 16),
                           mode='bilinear'), size=(224, 244))
        model(hq)
    ...
    for batch in tqdm(dataloader):
        ...
        if id > 10000:
            ...
            for i in inds:
                ...
                shutil.copy(result_paths[i], os.path.join(
                    output_path, f'{mag:05}_{output_batch}_{i}.jpg'))
            results = []
            result_paths = []
            id = 0
...
def build_kmeans():
    ...
    cluster_ids_x, cluster_centers = kmeans(
        latents, num_clusters=8, distance="euclidean", device=torch.device('cuda:0'))
    torch.save((cluster_ids_x, cluster_centers),
               '../k_means_instance_resnet.pth')
...
def use_kmeans():
    ...
    dataloader = get_image_folder_dataloader(
        batch_size, num_workers, target_size=224, shuffle=False)
    for i, batch in enumerate(tqdm(dataloader)):
        ...
        for b in range(pred.shape[0]):
            if pred[b] == 3:
                outpath = os.path.dirname(batch['HQ_path'][b]).replace(
                    '\\pn_coven\\cropped', '\\pn_coven\\modeling')
