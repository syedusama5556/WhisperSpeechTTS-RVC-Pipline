...
if __name__ == '__main__':
    ...
    parser.add_argument('-text', type=str, help='Text to speak.',
                        default='my father worked at the airport. he was air traffic control. he always knew when the president was flying in but was not allowed to tell anyone.')
    parser.add_argument('-opt_code_gen', type=str, help='Path to options YAML file used to train the code_gen model',
                        default='D:\\dlas\\options\\train_encoder_build_ctc_alignments.yml')
    parser.add_argument('-code_gen_model_name', type=str,
                        help='Name of the code_gen model in opt.', default='generator')
    parser.add_argument('-code_gen_model_path', type=str, help='Path to saved code_gen model weights',
                        default='D:\\dlas\\experiments\\train_encoder_build_ctc_alignments_medium\\models\\50000_generator_ema.pth')
    ...
    parser.add_argument('-output_path', type=str,
                        help='Where to store outputs.', default='../results/use_diffuse_tts')
    ...
    sr_cond = load_audio(
        conditioning_clips[args.cond], sr_sample_rate).to(args.device)
    ...
    with torch.no_grad():
        for p, code in enumerate([codes]):
            ...
            torchaudio.save(os.path.join(
                args.output_path, f'{p}_output_mean_base.wav'), output_base.cpu().squeeze(0), base_sample_rate)
            ...
            torchaudio.save(os.path.join(
                args.output_path, f'{p}_output_mean_sr.wav'), output.cpu().squeeze(0), sr_sample_rate)
