...
from dlas.scripts.audio.gen.speech_synthesis_utils import (
    do_spectrogram_diffusion, load_discrete_vocoder_diffuser, wav_to_mel)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-opt_diffuse', type=str, help='Path to options YAML file used to train the diffusion model',
    ...
    parser.add_argument('-diffusion_model_path', type=str, help='Diffusion model checkpoint to load.',
                        default='X:\\dlas\\experiments\\train_diffusion_vocoder_22k_level\\models\\15000_generator_ema.pth')
    parser.add_argument('-dvae_model_name', type=str,
                        help='Name of the DVAE model in opt.', default='dvae')
    parser.add_argument('-opt_gpt_tts', type=str, help='Path to options YAML file used to train the GPT-TTS model',
                        default='X:\\dlas\\experiments\\train_gpt_tts_unified.yml')
    parser.add_argument('-gpt_tts_model_name', type=str,
                        help='Name of the GPT TTS model in opt.', default='gpt')
    parser.add_argument('-gpt_tts_model_path', type=str, help='GPT TTS model checkpoint to load.',
                        default='X:\\dlas\\experiments\\train_gpt_tts_unified_large\\models\\45000_gpt_ema.pth')
    parser.add_argument('-opt_clip', type=str, help='Path to options YAML file used to train the CLIP model',
                        default='X:\\dlas\\experiments\\train_clip_text_to_voice.yml')
    parser.add_argument('-clip_model_name', type=str,
                        help='Name of the CLIP model in opt.', default='clip')
    parser.add_argument('-clip_model_path', type=str, help='CLIP model checkpoint to load.',
                        default='X:\\dlas\\experiments\\train_clip_text_to_voice_masking_bigger_batch\\models\\23500_clip_ema.pth')
    parser.add_argument('-opt_cond_clip', type=str, help='Path to options YAML file used to train the Conditioning CLIP model',
                        default='D:\\dlas\\options\\train_clip_cond_to_voice.yml')
    parser.add_argument('-cond_clip_model_name', type=str,
                        help='Name of the CLIP model in opt.', default='clip')
    parser.add_argument('-cond_clip_model_path', type=str, help='CLIP model checkpoint to load.',
                        default='D:\\dlas\\experiments\\train_clip_cond_to_voice\\models\\42000_clip_ema.pth')
    ...
    parser.add_argument('-text', type=str, help='Text to speak.',
                        default="I am a language model that has learned to speak.")
    parser.add_argument('-cond_preset', type=str,
                        help='Use a preset conditioning voice (defined above). Overrides cond_path.', default='simmons')
    parser.add_argument('-num_samples', type=int,
                        help='How many total outputs the autoregressive transformer should produce.', default=256)
    parser.add_argument('-num_batches', type=int,
                        help='How many batches those samples should be produced over.', default=16)
    parser.add_argument('-num_outputs', type=int,
                        help='Number of outputs to produce.', default=5)
    parser.add_argument('-output_path', type=str,
                        help='Where to store outputs.', default='../results/use_gpt_tts')
    ...
    gpt_opt['networks'][args.gpt_tts_model_name]['kwargs']['checkpointing'] = False
    gpt = load_model_from_config(preloaded_options=gpt_opt, model_name=args.gpt_tts_model_name,
                                 also_load_savepoint=False, load_path=args.gpt_tts_model_path).cuda().eval()
    ...
    for cond_path in cond_paths:
        ...
        conds.append(c)
    # And just use the last cond_wav for the diffusion model.
    ...
    with torch.no_grad():
        ...
        for b in tqdm(range(args.num_batches)):
            ...
            ctc_codes.extend(gpt.convert_attentions_to_aligned_codes(
                text, attentions, codes, conds.shape[1]))
        samples = torch.cat(samples, dim=0)

        print("Loading CLIP..")
        clip = load_model_from_config(args.opt_clip, model_name=args.clip_model_name,
                                      also_load_savepoint=False, load_path=args.clip_model_path).cuda().eval()
        cond_clip = load_model_from_config(args.opt_cond_clip, model_name=args.cond_clip_model_name,
                                           also_load_savepoint=False, load_path=args.cond_clip_model_path).cuda().eval()
        ...
        clip_results = clip(text.repeat(samples.shape[0], 1),
                            torch.full(
                                (samples.shape[0],), fill_value=text.shape[1]-1, dtype=torch.long, device='cuda'),
                            samples, torch.full(
                                (samples.shape[0],), fill_value=samples.shape[1]*1024, dtype=torch.long, device='cuda'),
                            return_loss=False)
        ...
        clip_results = clip_results * \
            (1-args.cond_clip_weight) + cond_clip_results * args.cond_clip_weight
        ...
        print("Loading DVAE..")
        dvae = load_model_from_config(
            args.opt_diffuse, args.dvae_model_name).cuda()
        print("Loading Diffusion Model..")
        diffusion = load_model_from_config(args.opt_diffuse, args.diffusion_model_name,
                                           also_load_savepoint=False, load_path=args.diffusion_model_path).cuda()
        ...
        for b in range(best_results.shape[0]):
            ...
            torchaudio.save(os.path.join(
                args.output_path, f'gpt_tts_output_{b}.wav'), wav.squeeze(0).cpu(), 22050)
