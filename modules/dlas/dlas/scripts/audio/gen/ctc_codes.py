...
from dlas.models.audio.tts.ctc_code_generator import CtcCodeGenerator
from dlas.models.audio.tts.transformer_diffusion_tts import \
    TransformerDiffusionTTS
from dlas.scripts.audio.gen.speech_synthesis_utils import (
    load_clvp, load_discrete_vocoder_diffuser, load_univnet_vocoder)
from dlas.trainer.injectors.audio_injectors import (
    TorchMelSpectrogramInjector, denormalize_mel)
...
def diffuse(text, codes, cond):
    ...
    bpe_tokenizer = VoiceBpeTokenizer(
    ...
    diffusion_model.load_state_dict(torch.load(
        'X:\\dlas\\experiments\\train_speech_diffusion_from_ctc_tfd5\\models\\26500_generator_ema.pth'))
    diffusion_model = diffusion_model.cuda().eval()
    with torch.no_grad():
        text_codes = torch.LongTensor(
            bpe_tokenizer.encode(text)).unsqueeze(0).to(codes.device)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(
        '../experiments/train_encoder_build_ctc_alignments_toy/models/76000_generator_ema.pth'))
    ...
    with torch.no_grad():
        ...
        for s in range(codes.shape[-1]):
            logits, confidences = model.inference(
                codes, ppads * mask, prepeats * mask)

            # prevent prediction of tokens that have already been predicted.
            confidences = confidences * mask.logical_not()
        ...
        decoded_codes = decode_ctc_metadata(
            codes[0], ppads[0], prepeats[0]).unsqueeze(0)
        cond = load_audio(
            'D:\\tortoise-tts\\tortoise\\voices\\train_dotrice\\1.wav', 22050).unsqueeze(0).cuda()
